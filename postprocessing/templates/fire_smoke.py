...
class FireSmoke(Template, Caching, IncidentExtract, PostProcessing):
    def __init__(
        self,
        image,
        split_image,
        ...
        """
        Fire Smoke Template Initilization
        Args:
        ...
        if self.rcon is not None:
            ...
            data = self.getbykey("firesmoke", self.camera_id, self.usecase_id)
            if data is None:
                self.initialize_cache()
            incident_cache= self.getbykey("incident_firesmoke", self.camera_id, self.usecase_id)
    ...
    def initialize_cache(self):
        ...
        self.setbykey("firesmoke", self.camera_id, self.usecase_id, cachedict)
    def initialize_incident_cache(self):
        ...
        cachedict["incidents"] = []
        self.setbykey("incident_firesmoke", self.camera_id, self.usecase_id, cachedict)
    ...
    def process_data(self,logger):
        ...
        all_detection["prediction_class"] = []
        print("=======caleed FireSmoke detection=====")
        # self.detection_init(self.detected_class,self.expected_class,self.image_time)
        all_detection, masked_image = self.process_steps()
        print("**********filtered dic******")
        print(all_detection)
        ...
        if cachedict is None or len(cachedict) == 0:
            self.initialize_cache()
            filtered_res_dict=all_detection
        else:
            print("======postprocessing called======")
            
            PostProcessing.__init__(self, all_detection, cachedict["detections"])
        ...
        if len(incident_dict)>0:
            ...
            cachedict_incident=self.getbykey("incident_firesmoke", self.camera_id, self.usecase_id)
