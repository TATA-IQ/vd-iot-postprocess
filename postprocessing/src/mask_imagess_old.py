...
class Masking:
    ...
    def get_mask_boundary(self, boundary_config):
        ...
        width=boundary_config[keys[0]]["image_width"]
        height= boundary_config[keys[0]]["image_height"]
        ...
        for gid in keys:
            ...
            dfs["x"]=x
            dfs["y"]=y
            color=x=boundary_config[gid]["boundary_coordinates"]["color"]
            img_new = cv2.fillPoly(img, pts =np.array([np.array(dfs[['x','y']].astype(int))]), color=ImageColor.getcolor(color)[::-1])
        return img_new
