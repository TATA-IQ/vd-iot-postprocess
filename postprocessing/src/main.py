...
class PostProcessingApp():
    ...
    def initialize(self,image,postprocess_config,topic_name,metadata,producer):
        ...
        print(f"{self.usecase_id}, {self.camera_id}, initialization time, {time_diff}")
    ...
    def call_template(self,filtered_res):
        if self.usecase_template_id==1:
            #template for ppe
            print("**********")
            print("template check")
            ppe=PPETemplate(filtered_res)
            filtered_res=ppe.filter_data()
            print("==========goin for cache====")
            _,filtered_res=self.verify_cache.filter_track_data(self.usecase_id,self.camera_id,filtered_res,self.image_name,byid=True,byoverlap=False)
            self.detection_output.extend(filtered_res)
        if self.usecase_template_id==2:
            #template for Fire and smoke
            _,filtered_res=self.verify_cache.filter_track_data(self.usecase_id,self.camera_id,filtered_res,self.image_name,byid=False,byoverlap=True)
            self.detection_output.extend(filtered_res)
        if self.usecase_template_id==8:

            bt=BrightnessTemplate()
            prcvd_brightness,hsv_bright=bt.process_template(self.frame)
            print("============perceived=======")
            print(prcvd_brightness)
            print("============HSV=============")
            print(hsv_bright)
            self.detection_output.extend([hsv_bright])
        # if self.usecase_template_id==3:
        #     print("======Running for fire smoke====")
        #     _,filtered_res=self.verify_cache.filter_track_data(self.usecase_id,self.camera_id,filtered_res,self.image_name,byid=False,byoverlap=True)
        #     self.detection_output.extend(filtered_res)
        ...
        # current_time=datetime.utcnow()
        # time_diff=(current_time-self.image_time).total_seconds()
        # self.logger.info(f"{self.usecase_id}, {self.camera_id}, usecase template time, {time_diff}")
    ...
    def create_request(self,model_type,model_framework):
        ...
        requestparams["model_config"]={'is_track':True,'conf_thres': 0.1,'iou_thres': 0.1,
    ...
    def process_step_model(self,url,classes,model_type,model_framework):
        ...
        try:
            detection_result=response["result"]
        except Exception as ex:
            detection_result=[]
