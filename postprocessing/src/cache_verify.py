        ...
        for track_img_data in track_cam_data:
            ...
            for det in track_img_det_data:
                if det["class_name"] not in class_dict_track_data.keys():
                    class_dict_track_data[det["class_name"]]=[{"xmin":det["xmin"],"ymin":det["ymin"],"xmax":det["xmax"],"ymax":det["ymax"]}]
                else:
                    class_dict_track_data[det["class_name"]].append({"xmin":det["xmin"],"ymin":det["ymin"],"xmax":det["xmax"],"ymax":det["ymax"]})
        ...
        for det in detection:
            if det["class_name"] in list(class_track_data.keys()):
                for trk in class_track_data[det["class_name"]]:
                    trk_cord=trk
                    overlap=self.overlap(trk["xmin"],trk["ymin"],trk["xmax"],trk["ymax"],det["xmin"],det["ymin"],det["xmax"],det["ymax"])
