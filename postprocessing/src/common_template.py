...
class Template(Model, DetectionProcess, Computation):
    ...
    def __init__(self, image, image_name, camera_id, image_time, steps, frame):
        ...
        Args:
            ...
            image_time (int ): image fetch time
            steps(dict): steps for the usecase to perform
            frame (np.array): numpy array of image
    ...
    def create_request(self, model_type, model_framework):
        '''
        Create request to call detection services based on model framework and model_type
        Args:
            model_type (str): type of model like object detection or ocr
            model_framework: yolov8, yolov5 or tensorflow
        returns:
            requestparams (dict): request query for model detection
        '''
        requestparams = {}
