        ...
        for brtin in self.incident_data.values():
            for det in self.detected_data:
                ...
                if det["class_name"] == "brightness":
                    ...
                    lower_limit = float(self.step_data["lower_limit"]) - (
                        (float(tolerence) / 100) * float(self.step_data["lower_limit"])
                    )

                    upper_limit = float(self.step_data["upper_limit"]) - (
                        (float(tolerence) / 100) * float(self.step_data["upper_limit"])
                    )
                ...
                self.expected_incident.append(
                    [
                        {
                            "id": count,
                            "incident_id": brtin["incident_id"],
                            "coordinate": {"x1": 0, "y1": 0, "x2": 0, "y2": 0},
                            "name": brtin["incident_name"],
                            "misc": misc,
                        }
                    ]
                )
